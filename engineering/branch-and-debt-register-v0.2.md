...
Latest M3 implementation evidence at this review point: `f196a2a`. M2 remains at `45ec3b3` in
...
| TD-011 | High | Broker service primitives are not yet assembled into the production executable or VM-qualified under LocalSystem. | The authenticated IPC components cannot yet provide a release-ready privileged service. | SCM-hosted Broker has bounded workers, frame/read/write deadlines, cancellation, overload rejection and shutdown tests. | Partial through `3efd705`, `93077ba`, `88a7d18` and `f196a2a`: overlapped I/O has total deadlines and cancellation, the fixed worker pool rejects overload and abandons timed-out execution lanes without spawning replacements, SCM state/control handling exists, and a production build cannot omit its embedded package identity; real component wiring, secure per-user activation under TD-016, recovery configuration and VM stop/restart proof remain activation blockers |
...
| TD-016 | High | A LocalSystem Broker has no secure per-user activation/bootstrap path for the owning Agent SID and fresh session capability. | A static secret, command-line secret or SYSTEM-owned data pipe would either expose control or prevent the interactive Agent from connecting; production service wiring would be unsafe. | A separate local activation protocol obtains the OS SID/session and process handle from the pipe, replacement-locks and verifies the exact packaged Agent image, creates a fresh bounded owner-only data pipe, rejects live-session takeover, and forces blocking/revocation on Agent exit; multi-session and restart behavior are VM-proven. | Open; existing data-pipe primitives correctly enforce a supplied owner SID plus 256-bit capability, but production provisioning of those values is intentionally absent |
