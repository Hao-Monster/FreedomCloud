...
Latest M3 implementation evidence at this review point: `4eb5721`. M2 remains at `45ec3b3` in
...
| TD-002 | High | No WFP callout/Broker data plane exists. | “Strict” cannot guarantee proxy-or-block. | Signed callout, authenticated Broker, TCP/UDP v4/v6, DNS/QUIC VM evidence. | Partial through `aa45bc5`, `358f933`, `bd68b0a` and `4eb5721`: driver attestation, canonical WFP enumeration, atomic real-BFE filter transactions, bounded IOCTL policy upload and a fail-closed four-callout kernel source path exist; provider/sublayer/callout management objects, listener lease, relay, driver build and VM proof remain |
| TD-003 | High | No persistent fail-closed recovery marker/filter set. | Broker/Core crash could allow physical-network fallback. | Persistent guard filters survive process death; restart/uninstall recovery proven. | Partial through `358f933` and `4eb5721`: the concrete backend uses a non-dynamic BFE session for transactional persistent guards, while the kernel source advertises only persistent fail-closed and blocks selected App-IDs whenever redirect is unavailable; the driver has not been built/loaded and crash/VM proof remains |
