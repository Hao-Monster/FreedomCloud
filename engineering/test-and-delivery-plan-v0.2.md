...
| M3 UDP admission gate | RED → GREEN / PASS (Rust behavior and C-source invariants only) | An active-attestation failure test and an uncertain-deactivation test first failed because the backend skipped rollback on snapshot error and removed filters even when gate revocation was unproven; a missing-guard test independently reproduced an incomplete inventory being accepted. Final gates pass with 148 normal and 156 `production-host` Broker tests, 0 failed/skipped; rustfmt, production Clippy with `-D warnings` and the optimized 722,432-byte Broker build pass. Tests cover filter-attest-before-open, lease-owner activation, active snapshot proof, revoke/attest-before-remove, conservative retention on uncertain revoke, gate closure before context drain, stable lease renewal and rejection of redirects without persistent guards. C compilation, real `DeviceIoControl`, WDK analysis, Driver Verifier races, production bridge health, packet capture/reinjection and VM traffic are `NOT RUN`. |
| M3 outbound UDP packet capture | RED → GREEN / PASS (C-source invariants plus Rust regression only) | The first target failed because no kernel capture record existed; review then found and removed an invalid `PsGetProcessExitStatus` call from the potentially `DISPATCH_LEVEL` classifier. Final gates pass with 148 normal and 156 `production-host` Broker tests, 0 failed/skipped; 14 kernel source tests plus the redirect/socket integration selectors pass, rustfmt and production Clippy with `-D warnings` pass, and the optimized 722,432-byte Broker SHA-256 is `D2E6B0A9E7908F661DB97609EDE49BE6F987D9F45C65076DE3DD4599F51DE199`. Source assertions cover exact outbound v4/v6 fields, one-NBL/one-NB and 16 KiB bounds, UDP-header consistency, no packet-path allocation/wait lock/APC-only liveness call, lease/PID/flow binding, completion-before-absorb and gate-close/rundown/purge/drain ordering. WDK headers/toolset are absent on this host, so C compilation, SDV/CodeQL, real Direct-I/O cancellation races, reply injection, production bridge ownership, throughput and Windows 11 VM traffic are `NOT RUN`. |
| M3 UDP injection provenance | RED → GREEN / PASS (C-source invariants plus Rust regression only) | The target first failed because no exact injection provenance existed. Final gates pass with 85 normal library tests and 156 `production-host` tests, 0 failed/skipped; 15 kernel source tests, rustfmt and production Clippy with `-D warnings` pass. Assertions cover exact address/port/flags/compartment/interface binding under the flow lock, v4/v6 transport-injection handle ordering and exact injection-context ownership before self bypass. SDK 10.0.26100 field availability was checked. WDK C compilation, packet construction/injection, asynchronous completion races, multi-destination `sendto`, connected UDP and VM traffic are `NOT RUN`; valid reply IOCTLs still return `STATUS_NOT_SUPPORTED`. |
| M3 UDP reply-flow index | RED → GREEN / PASS (C-source invariants plus Rust regression only) | The target first failed because reply validation had no bounded flow lookup. Review then found and fixed a collision miss that could return an unreferenced candidate. Final gates pass with 86 normal library tests and 157 `production-host` tests, 0 failed/skipped; 16 kernel source tests, rustfmt and production Clippy with `-D warnings` pass. Source assertions cover a fixed power-of-two 256-bucket index, atomic dual-list lifecycle, exact tuple/lease matching, structural-validation-before-lookup, collision-safe reference acquisition/release and fail-closed token exhaustion. WDK C compilation, runtime collision distribution/lock contention and VM execution are `NOT RUN`; reply injection remains disabled. |
| M3 driver reply replay window | RED → GREEN / PASS (C-source invariants plus Rust regression only) | The target first failed because the driver had no independent reply replay state. Review corrected the out-of-order path so it preserves the highest accepted sequence. Final gates pass with 87 normal library tests and 158 `production-host` tests, 0 failed/skipped; rustfmt and production Clippy with `-D warnings` pass. Assertions cover zero/duplicate/stale rejection, guarded forward/backward shifts, a 64-packet window, non-mutating validation and exact-flow-lock ownership. WDK C compilation, simultaneous reply IOCTL races and VM execution are `NOT RUN`; no sequence is committed while injection is disabled. |
| M3 UDP reply injection initiation | RED → GREEN / PASS (C-source invariants plus Rust regression only) | The target first failed because valid reply batches still returned `STATUS_NOT_SUPPORTED`. Final gates pass with 88 normal library tests and 161 `production-host` tests, 0 failed/skipped; rustfmt and production Clippy with `-D warnings` pass. Assertions cover the 256-operation hard cap, exact flow ownership, v4/v6 IP/UDP construction, NBL/MDL/context ownership, replay commit ordering, immediate-failure cleanup, completion cleanup and drain-before-flow/pool teardown. Source review checks the official WFP allocation/construction/injection contracts and fail-closed rejection of `ALE_CLASSIFY_REQUIRED`. WDK C compilation, SDV/CodeQL, real IOCTL/injection, completion failure injection, Driver Verifier, IPsec behavior, DNS/QUIC and Windows 11 VM traffic are `NOT RUN`; capability bits remain off. |
| M3 UDP injection health snapshot | RED → GREEN / PASS (C-source/ABI and Rust validation only) | The target first failed because completion status and monotonic accounting were absent. Final gates pass with 90 normal library tests and 163 `production-host` tests, 0 failed/skipped; rustfmt and normal/production Clippy with `-D warnings` pass. The versioned snapshot is exactly 168 bytes and validates attempted/succeeded/failed/in-flight consistency, the 256-operation ceiling, partial-batch count and last failure status. WDK C compilation, real asynchronous completion fault injection, teardown races and VM execution are `NOT RUN`; observation is not yet production gate revocation. |
| M3 Broker UDP injection health monitor | RED → GREEN / PASS (loopback/pure tests) | The monitor target initially failed to compile before health access existed. Final gates pass with 92 normal library tests and 165 `production-host` tests, 0 failed/skipped; rustfmt and normal/production Clippy with `-D warnings` pass. A loopback fake-driver test injects an asynchronous failure and proves the reusable bridge exits. Health is baselined per bridge, sampled at most once per 100 ms only while injection is unresolved, and idle bridges perform zero health IOCTLs. Production supervisor ownership, verified gate revocation, real driver faults and VM traffic are `NOT RUN`; capability bits remain off. |
| M3 production UDP bridge supervision | RED → GREEN / PASS (source, loopback and runtime tests) | The closed-gate pre-arm source target initially failed because receive admission required an already open UDP gate. Final gates pass with 92 normal library tests and 167 `production-host` tests, 0 failed/skipped. Focused tests cover accepted-I/O readiness, lease-owner/capacity-one pre-arm with gate-required replies, queue restart after purge, lease-mutation serialization, exact current/previous renewal visibility, revoke-before-bridge/listener teardown and Engine Actor worker-health fail-close. Rustfmt and normal/production Clippy with `-D warnings` pass. The optimized 795,648-byte Broker SHA-256 is `210FB5B7DCA383DB8CD9E27308CD039F4DEED10E206F2DD8E73A2762339A5F05`. Real `DeviceIoControl`, WDK C compilation, WFP execution, driver completion faults, VM traffic and recovery after actor exit are `NOT RUN`; capability bits remain off. |
| M3 recoverable Engine Actor | RED → GREEN / PASS (pure runtime tests) | The first regression failed because a supervised health fault completed fail-closed cleanup and then disconnected the Actor queue. A second regression failed because a `commitPolicy` already queued before the fault could be processed after cleanup and re-arm supervision. Final gates pass with 92 normal library tests and 168 total `production-host` tests, 0 failed/skipped. Tests prove cleanup-before-recovery, a maximum four-item stale-queue rejection, no 25 ms health/cleanup spin, continued status handling, re-arm only on a fresh authenticated commit attempt, orderly shutdown, and Actor exit with both errors when cleanup is unproven. Rustfmt and normal/production Clippy with `-D warnings` pass. The optimized 797,696-byte Broker SHA-256 is `878558A35ECFF82DBA4775F995206E8B4D0E79B9FBBD7D61B8B0EE85C6453EFE`. Automatic Agent retry, operator diagnostics, installed SCM recovery and Windows 11 VM fault execution are `NOT RUN`; capability bits remain off. |
| M3 Agent supervised forwarding retry | RED → GREEN / PASS (pure orchestration tests) | Both targets initially failed because an Armed orchestrator rejected a later Broker proof as the wrong phase. The consolidated Agent gate passes 44 tests, 0 failed/skipped. Tests prove that only a canonical persistent Blocking proof can enter `recovering`, the exact retained Core ingress revision/digest/generation/group is recommitted once without credential or Core mutation, an Armed proof settles recovery, and a second Blocking proof falls into force-block/revoke without another recommit. Rustfmt, Clippy with `-D warnings` and the optimized 616,960-byte Agent build pass; SHA-256 is `875EF9BCACC98849C0F697687F7A42250BEB118D18D203762153BB89733FDED5`. Production status scheduling, retry backoff/diagnostics, Broker/Agent service execution and Windows 11 VM fault injection are `NOT RUN`; capability bits remain off. |
| M3 Agent strict-recovery retry budget | PASS (pure deterministic state tests) | Commit `0251fe7` adds five-attempt 1/2/4/8/16-second capped backoff, explicit exhaustion diagnostics, overflow clamping and reset-on-success semantics. Four focused tests and all 48 Agent targets pass; rustfmt and normal Clippy with `-D warnings` pass. The primitive is not wired to UI/product policy, SCM recovery or VM execution; capability bits remain off. |
| M3 pinned-WDK driver compile/static analysis | RED → GREEN / PASS (local compile only) | Before the environment was installed, the driver failed at missing kernel headers. The pinned WDK then exposed compile-contract errors and seven analysis findings, including an actual IRQL defect where a fast mutex raised execution to APC_LEVEL before synchronous queue purge. Commit `b4ca23c` replaces that lock with a PASSIVE-level WDF wait lock, fixes WDK/SAL/integer contracts, disables implicit test signing and pins WDK/SDK `10.0.28000.2526`. The final Visual Studio 2026 x64 Release rebuild ran `/W4 /WX`, DriverMinimumRules and ApiValidator with 0 warnings/0 errors. The unsigned 37,888-byte `.sys` SHA-256 is `3DE8326A3BAF1FE786B81FE70DCF69F885817420A77638735DDD970B5F9F447A`. Broker passes 157 normal and 168 `production-host` tests; Contract 7, Agent 44, Core Go test/vet/build, rustfmt and normal/production Clippy also pass. Driver load/signing, real IOCTL/WFP traffic, Driver Verifier, HLK and VM performance remain `NOT RUN`; no local network or service state was changed. |
| M3 signed VM qualification tooling | PASS (local packaging tests) | Commit `87660fa` adds exact signed-file manifest generation, signed VM bundle assembly, read-only VM preflight, bounded 30-minute resource evidence collection and the M3 traffic/lifecycle/performance checklist. The automated Windows test passes positive manifest generation plus fail-closed unsigned-driver and wrong-Broker-embedded-manifest cases. PSScriptAnalyzer 1.25.0 reports 0 warning/error findings. A successful production bundle is `NOT RUN` because the required Microsoft-signed driver and release-signed Broker/Agent/Core do not exist yet; the tooling deliberately refuses the current unsigned driver and cannot weaken that external gate. |
| M3 WFP data-plane naming debt | RED → GREEN / PASS | The target plan test first failed to compile because `data_plane_filters` did not exist. Commit `b296c78` then renames the mixed TCP-redirect/UDP-flow-tracking plan, inventory, backend and Windows-engine APIs without changing behavior. Final gates pass 157 normal and 168 `production-host` Broker tests, rustfmt and normal/production Clippy with `-D warnings`. Exact inventory/order tests remain green; runtime WFP execution is unaffected and was not run. |
| M3 callout unregister outcome | RED → GREEN / PASS (source/static only) | The new source test first failed because unregister ignored WFP status and cleared IDs unconditionally. Commits `29aef8c`/`8d7da73` retain IDs until success/not-found, re-drain on `STATUS_DEVICE_BUSY`, retry at 10 ms and emit exponentially throttled kernel diagnostics with a saturating attempt counter before any dependent resource teardown. Final gates pass 158 normal and 169 `production-host` Broker tests, rustfmt and both Clippy gates. The pinned-WDK x64 Release rebuild runs `/W4 /WX`, DriverMinimumRules and ApiValidator with 0 warnings/errors; final reviewed unsigned 37,888-byte SHA-256 is `EDC4D4EE3DAC49039D9F082D9B652DE2D8F4F22C6BA9597DEADD1EDAA9BDD783`. Driver loading, status fault injection, Driver Verifier and VM stop timing are `NOT RUN`. |
